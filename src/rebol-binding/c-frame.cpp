//
// This is an experimental adaptation of Rebol 3's c-frame.c for use in the
...
// installation, the c-frame.c file was one that had no workaround without
...
{
    REBVAL value;
    value.flags.flags.resv = 0;
    value.flags.flags.exts = 0;
    value.flags.flags.opts = 0;
    value.flags.flags.type = 0;
    value.flags.header = 0;
    value.data.all.bits[0] = 0;
    value.data.all.bits[1] = 0;
    value.data.all.bits[2] = 0;
...
***********************************************************************/
{
	...
	if (NOT_END(words) || NOT_END(values))
        Debug_Fmt(
            ...
            NOT_END(words) ? "words" : "values", n, VAL_TYPE(words)
        );
}
...
***********************************************************************/
{
    ...
    Set_Root_Series(
        TASK_BUF_WORDS,
        Make_Block(100),
        const_cast<REBYTE *>(reinterpret_cast<REBYTE const *>("word cache"))
    ); // just holds words, no GC
}
