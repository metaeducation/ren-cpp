//
// The garbage collector must be hooked to have some kind of awareness of
// what objects the C++ code is holding onto.  There are many ways to go
// about that bookkeeping.  In the interest of advancing other issues,
// worrying about this is finessed by just disabling the GC...but this
// file could be modified to look at a list maintained by the C++ code.
//
...
// installation, the m-gc.c file was one that had no workaround without
...
#include "rebol/src/include/reb-evtypes.h"
}
...
***********************************************************************/
{
    ...
    // Make_Mem returns void*, needs cast
    GC_Infants = static_cast<REBSER**>(
        Make_Mem((MAX_SAFE_SERIES + 2) * sizeof(REBSER*))
    ); // extra

	Init_Pools(scale);

    // Make_Mem returns void*, needs cast
    Prior_Expand = static_cast<REBSER**>(
        Make_Mem(MAX_EXPAND_LIST * sizeof(REBSER*))
    );
